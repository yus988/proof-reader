  ...
  for (const d of diff) {
    ...
    if (d.type === "del") {
      ...
      for (let k = 0; k < sorted.length - 1; k++) {
        ...
        result.push({ type: d.type, text: subText, reason: d.type === "add" ? getReason("corr", absFrom, absTo) : null, bold });
      }
      corrPos += len;
...
export default function DiffViewer({ data, onBack }) {
  ...
  const copyAll = useCallback(() => { copyText(CORRECTED_MD); flashCopy("all"); }, [CORRECTED_MD, flashCopy]);
  ...
  const renderParagraph = (group, paraIdx, filter) => (
    <div key={paraIdx} onClick={(e) => copyParagraph(paraIdx, e)}
      style={{ padding: "2px 4px", marginBottom: 4, borderRadius: 4, cursor: "text",
        transition: "background 0.15s", background: copiedIdx === paraIdx ? C.copyFlash : "transparent", userSelect: "text",
      }} title="クリックで段落コピー（md）">
  ...
  const PanelHeader = ({ color, label }) => (
    <div style={{ padding: "10px 20px", background: C.surfaceAlt, borderBottom: `1px solid ${C.border}`,
      fontFamily: F, fontSize: 12, fontWeight: 700, color: C.muted, letterSpacing: "0.06em",
      display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ width: 8, height: 8, borderRadius: "50%", background: color, opacity: 0.6 }} />{label}
    </div>
  );
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: C.bg, fontFamily: F }}>
      <header style={{ background: C.header, color: C.headerText, padding: "14px 24px",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {onBack && <button onClick={onBack} style={{ background: "transparent", border: "none", color: C.headerText, cursor: "pointer", fontSize: 16, padding: "4px 8px", borderRadius: 4, opacity: 0.7 }}>{"\u2190"} 戻る</button>}
        ...
        <span style={{ fontSize: 12, fontWeight: 700, background: C.accent, color: "#fff", borderRadius: 20, padding: "4px 14px" }}>{changes} 箇所修正</span>
      </header>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        ...
        <div style={{ fontSize: 11, color: C.muted }}>クリックで段落コピー / 点線ホバーで修正理由</div>
      </div>
        ...
        {viewMode === "side" ? (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", flexShrink: 0 }}>
              <div style={{ flex: 1, borderRight: `1px solid ${C.border}` }}><PanelHeader color={C.delText} label="原文" /></div>
              <div style={{ flex: 1 }}><PanelHeader color={C.addText} label="校正後" /></div>
            </div>
            <div style={{ flex: 1, overflow: "auto", padding: "12px 0" }}>
              {paraGroups.map((g, pi) => (
                <div key={pi} style={{ display: "flex", borderBottom: `1px solid ${C.border}22`, minHeight: 20 }}>
                  {/* 原文パネル: 選択不可（ドラッグコピー時に校正後のみコピーされるように） */}
                  <div style={{ flex: 1, padding: "4px 22px", borderRight: `1px solid ${C.border}`, userSelect: "none", opacity: 0.85 }}>
            ...
            <div style={{ padding: "10px 20px", background: C.surfaceAlt, borderBottom: `1px solid ${C.border}`,
              ...
              <span style={{ display: "flex", gap: 10, fontSize: 11, fontWeight: 500, letterSpacing: 0 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ display: "inline-block", width: 12, height: 12, borderRadius: 2, background: C.delBg, border: `1px solid ${C.delText}33` }} /> 削除</span>
                <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ display: "inline-block", width: 12, height: 12, borderRadius: 2, background: C.addBg, border: `1px solid ${C.addText}33` }} /> 追加</span>
              </span>
            </div>
      ...
      <div style={{ padding: "10px 24px", background: copiedIdx === "all" ? C.copyFlash : C.surface,
        borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "flex-end", flexShrink: 0, transition: "background 0.15s" }}>
        <button onClick={copyAll} style={{
          fontFamily: F, fontSize: 13, fontWeight: 600, padding: "6px 18px", borderRadius: 6, border: "none", cursor: "pointer",
          background: copiedIdx === "all" ? C.addText : C.accent, color: "#fff",
      ...
      {tooltip && (
        <div style={{ position: "fixed", left: tooltip.x, top: tooltip.y, transform: "translate(-50%, -100%)",
          ...
          maxWidth: 280, whiteSpace: "normal", wordBreak: "keep-all", overflowWrap: "break-word", zIndex: 9999, pointerEvents: "none" }}>
          {tooltip.reason}
          <div style={{ position: "absolute", bottom: -6, left: "50%", transform: "translateX(-50%)",
            width: 0, height: 0, borderLeft: "6px solid transparent", borderRight: "6px solid transparent", borderTop: `6px solid ${C.tipBg}` }} />
        </div>
      )}
      ...
      {copiedIdx !== null && copiedIdx !== "all" && (
        <div style={{ position: "fixed", bottom: 80, left: "50%", transform: "translateX(-50%)",
          ...
          padding: "8px 20px", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.2)", zIndex: 9999 }}>{"\u2713"} 段落をコピーしました</div>
      )}
    </div>
  );
}
